...
\title{Intro R Workshop: Data Manipulation, Analysis, and Visualisation\thanks{Replication files are available on the author's GitHub account}}
\usepackage{etoolbox}
\makeatletter
...
\subsection{Venue, date and time}\label{venue-date-and-time}}
...
\subsubsection*{Day 1 -- In the Beginning}\label{day-1-in-the-beginning}}
\addcontentsline{toc}{subsubsection}{Day 1 -- In the Beginning}
...
\subsubsection*{Day 2 -- Show and tell}\label{day-2-show-and-tell}}
\addcontentsline{toc}{subsubsection}{Day 2 -- Show and tell}
...
\subsubsection*{Day 3 -- Going deeper}\label{day-3-going-deeper}}
\addcontentsline{toc}{subsubsection}{Day 3 -- Going deeper}
...
\subsubsection*{Day 4 -- The Enlightened Researcher}\label{day-4-the-enlightened-researcher}}
\addcontentsline{toc}{subsubsection}{Day 4 -- The Enlightened Researcher}
...
\subsubsection*{Day 5 -- The world is yours}\label{day-5-the-world-is-yours}}
\addcontentsline{toc}{subsubsection}{Day 5 -- The world is yours}
...
\subsection{About this Workshop}\label{about-this-workshop}}
...
\subsubsection{Some negatives of using R}\label{some-negatives-of-using-r}}
...
\subsubsection{The challenge: learning to program in R}\label{the-challenge-learning-to-program-in-r}}
...
\subsection{Style and code conventions}\label{style-and-code-conventions}}
...
\subsection{Exercise: It which shall not be named}\label{exercise-it-which-shall-not-be-named}}
...
\subsubsection{General settings}\label{general-settings}}
...
\subsubsection{Customising appearance}\label{customising-appearance}}
...
\subsubsection{Configuring panes}\label{configuring-panes}}
...
\subsection{The Rproject}\label{the-rproject}}
...
The most common functions used in R are contained within the \textbf{\texttt{base}} package; this makes R useful `out of the box.' However, there is extensive additional functionality that is being expanded all the time through the use of packages. Packages are simply collections of code called functions that automate complex mathematical or statistical tasks. One of the most useful features of R is that users are continuously developing new packages and making them available for free. You can find a comprehensive list of available packages on the \href{https://cran.r-project.org/web/packages/}{CRAN website}. There are currently (\texttt{2021-06-25}) \texttt{17785} packages available for R!
...
\subsection{The panes of RStudio}\label{the-panes-of-rstudio}}
...
\subsubsection{Environment and History panes}\label{environment-and-history-panes}}
...
\subsubsection{Files, Plots, Packages, Help, and Viewer panes}\label{files-plots-packages-help-and-viewer-panes}}
...
\subsection{Create and account and a repository}\label{create-and-account-and-a-repository}}
...
\subsection{Git configuring (activation) and connecting to a remote repository}\label{git-configuring-activation-and-connecting-to-a-remote-repository}}
...
\subsection{Clone the new GitHub repository to your computer via RStudio.}\label{clone-the-new-github-repository-to-your-computer-via-rstudio.}}
...
\subsection{Open a Pull Request}\label{open-a-pull-request}}
...
\section{An R workflow}\label{workflow}}
...
\subsubsection{Preparing data for R}\label{preparing-data-for-r}}
...
\subsubsection{Converting data}\label{converting-data}}
...
\subsubsection{Loading a file}\label{loading-a-file}}
...
\subsubsection{Examine your data}\label{examine-your-data}}
...
\subsubsection{Tidyverse sneak peek}\label{tidyverse-sneak-peek}}
...
\subsubsection{Summary of all variables in a dataframe}\label{summary-of-all-variables-in-a-dataframe}}
...
\subsubsection{Summary statistics by variable}\label{summary-statistics-by-variable}}
...
\subsubsection{More complex calculations}\label{more-complex-calculations}}
...
\subsection{Clearing the memory}\label{clearing-the-memory}}
...
\section{\texorpdfstring{Graphics with \textbf{\texttt{ggplot2}}}{Graphics with ggplot2}}\label{graphics}}
...
\subsection{Example figures}\label{example-figures}}
...
\subsection{\texorpdfstring{Basics of \textbf{\texttt{ggplot2}}}{Basics of ggplot2}}\label{basics-of-ggplot2}}
...
\subsection{\texorpdfstring{To \texttt{aes()} or not to \texttt{aes()}, that is the question}{To aes() or not to aes(), that is the question}}\label{to-aes-or-not-to-aes-that-is-the-question}}
...
\section{Faceting Figures}\label{faceting}}
...
\subsection{Faceting one figure}\label{faceting-one-figure}}
...
\subsection{New figure types}\label{new-figure-types}}
...
\subsection{Gridding figures}\label{gridding-figures}}
...
\hypertarget{brewing}{%
\section{\texorpdfstring{Brewing colours in \textbf{\texttt{ggplot2}}}{Brewing colours in ggplot2}}\label{brewing}}
...
\hypertarget{r-data}{%
\subsection{R Data}\label{r-data}}
...
\subsection{\texorpdfstring{\textbf{\texttt{RColorBrewer}}}{RColorBrewer}}\label{rcolorbrewer}}
...
\subsection{Make your own palettes}\label{make-your-own-palettes}}
...
\subsection{Use your own palettes}\label{use-your-own-palettes}}
...
\subsection{DIY figures}\label{diy-figures}}
...
\hypertarget{mapping}{%
\section{\texorpdfstring{Mapping with \textbf{\texttt{ggplot2}}}{Mapping with ggplot2}}\label{mapping}}
...
\subsection{A new concept?}\label{a-new-concept}}
...
\subsection{Land mask}\label{land-mask}}
...
\subsection{Force lon/lat extent}\label{force-lonlat-extent}}
...
\subsection{Ocean temperature}\label{ocean-temperature}}
...
\section{Mapping with style}\label{mapping_style}}
...
\subsection{Default maps}\label{default-maps}}
...
\begin{Highlighting}[]
\FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{borders}\NormalTok{(}\AttributeTok{col =} \StringTok{"black"}\NormalTok{, }\AttributeTok{fill =} \StringTok{"cornsilk"}\NormalTok{, }\AttributeTok{size =} \FloatTok{0.2}\NormalTok{) }\SpecialCharTok{+} \CommentTok{\# The global shape file}
...
\NormalTok{sa\_1 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{borders}\NormalTok{(}\AttributeTok{size =} \FloatTok{0.2}\NormalTok{, }\AttributeTok{fill =} \StringTok{"cornsilk"}\NormalTok{, }\AttributeTok{colour =} \StringTok{"black"}\NormalTok{) }\SpecialCharTok{+}
...
\subsection{Specific labels}\label{specific-labels}}
...
\NormalTok{sa\_2 }\OtherTok{\textless{}{-}}\NormalTok{ sa\_1 }\SpecialCharTok{+}
  ...
  \FunctionTok{annotate}\NormalTok{(}\StringTok{"text"}\NormalTok{, }\AttributeTok{label =} \StringTok{"Indian}\SpecialCharTok{\textbackslash{}n}\StringTok{Ocean"}\NormalTok{, }
           ...
           \AttributeTok{colour =} \StringTok{"red4"}\NormalTok{)}
\NormalTok{sa\_2}
\end{Highlighting}
\end{Shaded}
...
\subsection{Scale bars}\label{scale-bars}}
...
\NormalTok{sa\_3 }\OtherTok{\textless{}{-}}\NormalTok{ sa\_2 }\SpecialCharTok{+}
  \FunctionTok{scalebar}\NormalTok{(}\AttributeTok{x.min =} \DecValTok{22}\NormalTok{, }\AttributeTok{x.max =} \DecValTok{26}\NormalTok{, }\AttributeTok{y.min =} \SpecialCharTok{{-}}\DecValTok{36}\NormalTok{, }\AttributeTok{y.max =} \SpecialCharTok{{-}}\DecValTok{35}\NormalTok{, }\CommentTok{\# Set location of bar}
           \AttributeTok{dist =} \DecValTok{200}\NormalTok{, }\AttributeTok{dist\_unit =} \StringTok{"km"}\NormalTok{, }\AttributeTok{height =} \FloatTok{0.3}\NormalTok{, }\AttributeTok{st.dist =} \FloatTok{0.8}\NormalTok{, }\AttributeTok{st.size =} \DecValTok{4}\NormalTok{, }\CommentTok{\# Set particulars}
           \AttributeTok{transform =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{border.size =} \FloatTok{0.2}\NormalTok{, }\AttributeTok{model =} \StringTok{"WGS84"}\NormalTok{) }\SpecialCharTok{+} \CommentTok{\# Set appearance}
...
\subsection{Session inf}\label{session-inf}}
...
\section{Mapping with OpenStreetMap}\label{mapping_google}}
...
\subsection{\texorpdfstring{\textbf{\texttt{OpenStreetMap}}}{OpenStreetMap}}\label{openstreetmap}}

The package we will need for this tut is \textbf{\texttt{OpenStreetMap}}. It is a bit beefy so let's get started on installing it now. Once it's done we will activated the packages we need and load some site points for mapping.
...
\subsection{Mapping Cape Point}\label{mapping-cape-point}}

We will be creating our OpenStreetMap map in two steps. The first step is to use the \texttt{openmap()} function to define the area we want a map of, as well as what sort of map we want. In Google Maps and Google Earth we can switch from satellite view to map view, and we may do that in R and OpenStreetMap as well. For now we are only going to use the satellite view so that we may insert out own labels. But if we look in the help file we may see a description of all the different map types available. There are a bunch!

The \texttt{openmap()} function relies on a healthy internet connection to run. The downloading of OpenStreetMap maps can be a tenuous process without a stable connection. In the same sequence of steps we also add some points to indicate the spatial location of the kelp sampling sites (for the \emph{Laminaria} and \emph{Ecklonia} data), and the same annotations for the Atlantic Ocean that we first saw in Chapter 9. Here is how:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# load the location of the study sites}
\FunctionTok{load}\NormalTok{(}\StringTok{"data/cape\_point\_sites.RData"}\NormalTok{)}

\CommentTok{\# define the spatial extent to OpenStreetMap}
\NormalTok{lat1 }\OtherTok{\textless{}{-}} \SpecialCharTok{{-}}\FloatTok{34.5}\NormalTok{; lat2 }\OtherTok{\textless{}{-}} \SpecialCharTok{{-}}\FloatTok{33.5}\NormalTok{; lon1 }\OtherTok{\textless{}{-}} \DecValTok{18}\NormalTok{; lon2 }\OtherTok{\textless{}{-}} \DecValTok{19}

\FunctionTok{library}\NormalTok{(OpenStreetMap)}

\CommentTok{\# other \textquotesingle{}type\textquotesingle{} options are "osm", "maptoolkit{-}topo", "bing", "stamen{-}toner",}
\CommentTok{\# "stamen{-}watercolor", "esri", "esri{-}topo", "nps", "apple{-}iphoto", "skobbler";}
\CommentTok{\# play around with \textquotesingle{}zoom\textquotesingle{} to see what happens; 10 seems just right to me}
\NormalTok{sa\_map }\OtherTok{\textless{}{-}} \FunctionTok{openmap}\NormalTok{(}\FunctionTok{c}\NormalTok{(lat2, lon1), }\FunctionTok{c}\NormalTok{(lat1, lon2), }\AttributeTok{zoom =} \DecValTok{10}\NormalTok{,}
                  \AttributeTok{type =} \StringTok{"esri{-}topo"}\NormalTok{, }\AttributeTok{mergeTiles =} \ConstantTok{TRUE}\NormalTok{)}

\CommentTok{\# reproject onto WGS84}
\NormalTok{sa\_map2 }\OtherTok{\textless{}{-}} \FunctionTok{openproj}\NormalTok{(sa\_map)}

\CommentTok{\# use instead of \textquotesingle{}ggplot()\textquotesingle{}}
\NormalTok{sa\_map2\_plt }\OtherTok{\textless{}{-}}\NormalTok{ OpenStreetMap}\SpecialCharTok{::}\FunctionTok{autoplot.OpenStreetMap}\NormalTok{(sa\_map2) }\SpecialCharTok{+} 
  \FunctionTok{annotate}\NormalTok{(}\StringTok{"text"}\NormalTok{, }\AttributeTok{label =} \StringTok{"Atlantic}\SpecialCharTok{\textbackslash{}n}\StringTok{Ocean"}\NormalTok{, }
           \AttributeTok{x =} \FloatTok{18.2}\NormalTok{, }\AttributeTok{y =} \SpecialCharTok{{-}}\FloatTok{33.8}\NormalTok{, }\AttributeTok{size =} \FloatTok{5.0}\NormalTok{, }\AttributeTok{angle =} \SpecialCharTok{{-}}\DecValTok{60}\NormalTok{, }\AttributeTok{colour =} \StringTok{"navy"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{data =}\NormalTok{ cape\_point\_sites,}
             \FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ lon }\SpecialCharTok{+} \FloatTok{0.002}\NormalTok{, }\AttributeTok{y =}\NormalTok{ lat }\SpecialCharTok{{-}} \FloatTok{0.007}\NormalTok{), }\CommentTok{\# slightly shift the points}
             \AttributeTok{colour =} \StringTok{"red"}\NormalTok{, }\AttributeTok{size =}  \FloatTok{2.5}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{xlab}\NormalTok{(}\StringTok{"Longitude (°E)"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"Latitude (°S)"}\NormalTok{)}
\NormalTok{sa\_map2\_plt}
\end{Highlighting}
\end{Shaded}

\begin{figure}
\centering
\includegraphics{10-mapping_google_files/figure-latex/openstreetmap-cp-ghost-1.pdf}
\caption{\label{fig:openstreetmap-cp-ghost}Google map of Cape Point}
\end{figure}

The process seen in the code above is to treat the OpenStreetMap data we downloaded as though it is just any ordinary \textbf{\texttt{ggplot2}} object. The same as the ones we created yesterday and today. For this reason we may use \texttt{+} to add new lines of conventional \textbf{\texttt{ggplot2}} code to the map drawn by \texttt{OpenStreetMap::autoplot.OpenStreetMap()} in order to show site annotations, locations, etc.
...
\subsection{Site labels}\label{site-labels}}

The previous figure shows our sites along Cape Point, which is great, but which site is which?! We need site labels. This is a relatively straightforward process, so we've given a complicated example of how to do so here. We already loaded the data in the file \texttt{data/cape\_point\_sites.RData}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sa\_map3\_plt }\OtherTok{\textless{}{-}}\NormalTok{ sa\_map2\_plt }\SpecialCharTok{+}
  \FunctionTok{geom\_text}\NormalTok{(}\AttributeTok{data =}\NormalTok{ cape\_point\_sites, }\CommentTok{\# Choose dataframe}
            ...
            \AttributeTok{size =} \DecValTok{3}\NormalTok{, }\AttributeTok{colour =} \StringTok{"salmon"}\NormalTok{) }\SpecialCharTok{+} \CommentTok{\# Adjust appearance}
  \FunctionTok{ggtitle}\NormalTok{(}\StringTok{"Cape Point kelp sampling sites"}\NormalTok{)}
\NormalTok{sa\_map3\_plt}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{fig:ggmap-2}Google map of Cape Point with site points labeled}
\end{figure}

\hypertarget{diy-maps}{%
\subsection{DIY maps}\label{diy-maps}}
...
\subsection{Gathering and spreading}\label{gathering-and-spreading}}
...
\subsubsection{\texorpdfstring{Separating dates using \texttt{mutate()}}{Separating dates using mutate()}}\label{separating-dates-using-mutate}}
...
\subsection{But why though?}\label{but-why-though}}
...
\hypertarget{tidier}{%
\section{Tidier data}\label{tidier}}
...
\subsection{\texorpdfstring{Arrange observations (rows) with \texttt{arrange()}}{Arrange observations (rows) with arrange()}}\label{arrange-observations-rows-with-arrange}}
...
\subsection{\texorpdfstring{Filter observations (rows) with \texttt{filter()}}{Filter observations (rows) with filter()}}\label{filter-observations-rows-with-filter}}
...
\subsection{\texorpdfstring{Select variables (columns) with\texttt{select()}}{Select variables (columns) withselect()}}\label{select-variables-columns-withselect}}
...
\subsection{\texorpdfstring{Create new variables (columns) with \texttt{mutate()}}{Create new variables (columns) with mutate()}}\label{create-new-variables-columns-with-mutate}}
...
\subsection{\texorpdfstring{Summarise variables (columns) with \texttt{summarise()}}{Summarise variables (columns) with summarise()}}\label{summarise-variables-columns-with-summarise}}
...
\hypertarget{tidiest}{%
\section{Tidiest data}\label{tidiest}}
...
\subsection{\texorpdfstring{Group observations (rows) by variables (columns) with \texttt{group\_by()}}{Group observations (rows) by variables (columns) with group\_by()}}\label{group-observations-rows-by-variables-columns-with-group_by}}
...
\subsubsection{Grouping by multiple variables}\label{grouping-by-multiple-variables}}
...
\subsubsection{Ungrouping}\label{ungrouping}}
...
\subsection{\texorpdfstring{Chain functions with the pipe (\texttt{\%\textgreater{}\%})}{Chain functions with the pipe (\%\textgreater\%)}}\label{chain-functions-with-the-pipe}}
...
\subsection{Group all the functions!}\label{group-all-the-functions}}
...
\subsection{\texorpdfstring{Pipe into \textbf{\texttt{ggplot2}}}{Pipe into ggplot2}}\label{pipe-into-ggplot2}}
...
\subsection{Additional useful functions}\label{additional-useful-functions}}
...
\subsubsection{\texorpdfstring{Rename variables (columns) with \texttt{rename()}}{Rename variables (columns) with rename()}}\label{rename-variables-columns-with-rename}}
...
\subsubsection{\texorpdfstring{Create a new dataframe for a newly created variable (column) with \texttt{transmute()}}{Create a new dataframe for a newly created variable (column) with transmute()}}\label{create-a-new-dataframe-for-a-newly-created-variable-column-with-transmute}}
...
\subsubsection{\texorpdfstring{Count observations (rows) with \texttt{n()}}{Count observations (rows) with n()}}\label{count-observations-rows-with-n}}
...
\subsubsection{\texorpdfstring{Select observations (rows) by number with \texttt{slice()}}{Select observations (rows) by number with slice()}}\label{select-observations-rows-by-number-with-slice}}
...
\subsection{\texorpdfstring{The new age \emph{redux}}{The new age redux}}\label{the-new-age-redux}}
